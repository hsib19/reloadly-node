# Get Gift Card Products by ISO Code

The Reloadly Node SDK allows you to fetch available gift card products for a specific country using its ISO code.
...
| ------------- | ------ | -------- | ------------------------------------------------------------------------- | ------- |
| `countryCode` | String | Yes      | The ISO code of the country you want to display available gift cards for. | US      |
...
async function main() {
  try {
    const productsByIso = await reloadlySDK.giftcards.getProductsByIsoCode('US');

    console.log('Gift card products for US:', productsByIso);
  } catch (error) {
...
- Path parameter `countryCode` is **required** to identify the country for which gift card products should be retrieved.
...
- Useful for retrieving all available gift card products for a specific country before purchase or integration into your application.
