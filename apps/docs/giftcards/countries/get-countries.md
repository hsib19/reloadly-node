...
The Reloadly Node SDK allows you to easily fetch the list of supported countries for Giftcards, GiftCards, or Utility Payments.
...
async function main() {
  try {
    const countries = await reloadly.giftcards.getCountries();
...
- The SDK applies the correct `Accept` headers required for Giftcards requests.
